...
object ErrorHandler {
     ...
     */
    fun handleBluetoothError(
        context: Context,
        error: Throwable
    ): String {
        val message = when {
            error.message?.contains("permission", ignoreCase = true) == true -> 
                "Bluetooth permission is required for sensor connectivity"
            error.message?.contains("disabled", ignoreCase = true) == true -> 
                "Please enable Bluetooth in device settings"
            error.message?.contains("not found", ignoreCase = true) == true -> 
                "Bluetooth device not found. Please check pairing"
            else -> "Bluetooth connection error. Please try again"
        }

        Log.e(TAG, "Bluetooth Error: ${error.message}", error)
        Toast.makeText(context, message, Toast.LENGTH_LONG).show()
    ...
    fun handleSyncError(
        ...
        Log.w(TAG, "Sync Error: ${error.message}", error)
    ...
    fun handleAuthError(
        ...
        Log.e(TAG, "Auth Error: ${error.message}", error)
        Toast.makeText(context, message, Toast.LENGTH_LONG).show()
